...
export function startGame({ humans = 2, seed = 1, aiEnabled = false, guests = 0 } = {}) {
  const clock = { now: 0 };
  const game = new Game({ scenario, code: "TEST", seed, aiEnabled, now: clock.now });
  const people = [];
...
export function secretsFor(characterIds) {
  const texts = [];
  for (const c of scenario.characters.filter((x) => characterIds.includes(x.id))) {
    texts.push(c.dossier.secret, ...c.dossier.knows, c.reveal, c.gossip);
  }
  for (const clue of Object.values(scenario.clues)) texts.push(clue.text);
  texts.push(scenario.envelope.back, ...scenario.dictaphone.transcript.filter((l) => !l.startsWith("[")));
  for (const line of Object.values(scenario.mischief.openingWhisper)) texts.push(line);
  return texts;
}
